...
The web UI at `http://127.0.0.1:9180/` renders an "Update Hermes" banner above the chat when a newer stable official tag carries signature bytes (`update_available`). If the newest publication is unsigned, the first-class button selects the newest earlier signed release that is still newer than the installed version; `published_latest*` preserves the newer unsigned publication and `selected_signed_fallback=true` explains the choice. If no newer signed release exists, `update_blocked_reason=official_tag_unsigned` (or `official_tag_signature_unknown`) disables Update truthfully. Pin lists signed tags only. F4 still verifies the authorized signer before any checkout. Cluster-wide polling is omitted because MS4 runs per-spirit.

`GET /api/v1/hermes/version` also returns `installed_relation` (`older`/`current`/`newer`/`unknown`) and `operator_state`. Gateway startup first fails a persisted running job only after the updater lock proves ownerless; the read-only GET does not acquire that lock. Startup and version refresh then supersede a failed last-update with a verified current/newer no-op when installed ≥ selected latest. Installed < the newest publication with no newer signed candidate is `operator_state=blocked` as the primary banner; the prior failed job remains in `last_update` audit. The original failure remains in `progress` and `superseded_failure` when a current/newer no-op supersedes presentation; the UI follows `operator_state` rather than hiding a still-failed snapshot. Unknown/malformed versions fail closed. `POST /api/v1/hermes/update` with no target resolves the newest newer signed stable release; an explicit lower target is refused as a downgrade. Provenance, origin, signature, platform, arch, and anti-downgrade policy are unchanged.
